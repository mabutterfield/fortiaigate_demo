...
| Functional validation | Supported after GUI flow setup | Runs against the deployed AWS environment | Runs against the local lab | Confirm every installed scenario path behaves as declared | Installed scenarios, matching FortiAIGate GUI flows, and reachable chatbot | `python3 -m functional_test` | `python3 -m functional_test` | Sends synthetic scenario traffic and records expected results over total results |
