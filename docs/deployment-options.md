...
| AWS k3s GPU instance size | `g4dn.4xlarge` budget default | Profile initialization offers the default T4, supported `g6.4xlarge`/`g6.8xlarge` L4 sizes, or a custom type | Not applicable; local setup selects existing GPU UUIDs | EC2 cost, regional capacity/quota, supported-GPU validation, and CPU/RAM headroom | EC2 quota and regional offering; FortiAIGate license | Choose during profile initialization or set `instance_type` in ignored `terraform/aws-ec2-k3s/99-local.auto.tfvars` | `terraform -chdir=terraform/aws-ec2-k3s output instance_type`; `ansible-playbook -i cloud ansible/playbooks/status_fortiaigate.yml` | T4 is not a supported FortiAIGate GPU family; changing an existing type can stop/restart the host and lose ephemeral instance-store k3s data |
