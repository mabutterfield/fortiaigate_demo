...
- `test_model_direct.yml` runs a direct Bedrock or Ollama model test; Bedrock signing is handled by `tests/bedrock_direct_test.py`.
- `test_fortiaigate_chat.yml` runs the first external chat completion test after a guard/provider is configured; request execution is handled by `tests/fortiaigate_chat_test.py`.
