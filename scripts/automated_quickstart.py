...
def requested_appliance_keys(
    args: argparse.Namespace,
    *,
    use_persisted_aws_intent: bool = True,
) -> list[str]:
    """Return AWS-profile desired appliances, with command-line flags taking precedence."""
    requested: list[str] = []
    disable_all = bool(getattr(args, "no_appliances", False))
    include_all = bool(getattr(args, "include_appliances", False))
    fortigate_requested = include_all or bool(getattr(args, "include_fortigate", False))
    fortiweb_requested = include_all or bool(getattr(args, "include_fortiweb", False))
    if (
        not disable_all
        and not bool(getattr(args, "no_fortigate", False))
        and (fortigate_requested or not use_persisted_aws_intent or appliance_enabled_from_tfvars("fortigate"))
    ):
        requested.append("fortigate")
    if (
        not disable_all
        and not bool(getattr(args, "no_fortiweb", False))
        and (fortiweb_requested or not use_persisted_aws_intent or appliance_enabled_from_tfvars("fortiweb"))
    ):
...
def selected_appliance_keys(args: argparse.Namespace) -> list[str]:
    return requested_appliance_keys(args)


def selected_local_appliance_keys(args: argparse.Namespace) -> list[str]:
    requested = set(requested_appliance_keys(args, use_persisted_aws_intent=False))
...
def configure_appliance_tfvars(appliance_keys: list[str]) -> None:
    ...
    for appliance_key in appliance_keys:
        print(f"{appliance_key}: using the persistent profile intent.")


def configure_disabled_appliance_tfvars(args: argparse.Namespace) -> None:
    """Report transient CLI exclusions without overwriting the persistent profile."""
    disabled = explicitly_disabled_appliance_keys(args)
    if not disabled:
        return
    print_header("Appliance Command-Line Overrides")
    for appliance_key in sorted(disabled):
        print(f"{appliance_key}: skipped for this quickstart run; persistent profile unchanged.")
...
def terraform_apply(module_path: str, auto_approve: bool, variables: list[str] | None = None) -> None:
...
def run_terraform(
    ...
    *,
    ...
    transient_appliance_enables: set[str] | None = None,
) -> None:
    ...
    for appliance_key in appliance_keys:
        ...
        print_header(f"Terraform: {label}")
        terraform_init_validate(module_path)
        variables = [f"{appliance_key}_enabled=true"] if appliance_key in (transient_appliance_enables or set()) else []
        terraform_apply(module_path, auto_approve, variables)
...
def main() -> None:
    ...
    requested_appliances = requested_appliance_keys(
        args,
        use_persisted_aws_intent=args.target == "aws",
    )
    ...
    if args.skip_terraform:
        ...
        run_terraform(
            ...
            profile,
            region,
            noninteractive_import=args.yolo,
            transient_appliance_enables=explicitly_requested_appliance_keys(args),
        )
    prompt_ec2_status(
        profile,
        region,
