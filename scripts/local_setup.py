...
def prompt_fortigate_appliance(
    *,
    ...
    configured_default = generated_scalar(generated_defaults, "fortigate_local_enabled")
    default_enabled = configured_default != "false"
    if not prompt_yes_no("Configure an existing local FortiGate now?", default_enabled):
        ...
        return ApplianceBootstrapResult(
            key=key,
            enabled=False,
            generated_vars={"fortigate_local_enabled": "false"},
            secret_content="",
            access_cidrs=[],
        )
...
def prompt_fortiweb_appliance(
    *,
    ...
    configured_default = generated_scalar(generated_defaults, "fortiweb_local_enabled")
    default_enabled = configured_default != "false"
    if not prompt_yes_no("Configure an existing local FortiWeb now?", default_enabled):
        ...
        return ApplianceBootstrapResult(
            key=key,
            enabled=False,
            generated_vars={"fortiweb_local_enabled": "false"},
            secret_content="",
            access_cidrs=[],
        )
...
def main() -> None:
    ...
    if args.non_interactive:
        preserved_results: list[ApplianceBootstrapResult] = []
        for key, inventory_path in (
            ("fortigate", FORTIGATE_LOCAL_INVENTORY),
            ("fortiweb", FORTIWEB_LOCAL_INVENTORY),
        ):
            configured = generated_scalar(local_defaults, f"{key}_local_enabled")
            if not configured and inventory_path.exists():
                configured = "true"
            if configured in {"true", "false"}:
                preserved_vars = {
                    item_key: str(value)
                    for item_key, value in local_defaults.items()
                    if isinstance(value, str)
                    and (
                        item_key.startswith(f"{key}_")
                        or (key == "fortigate" and item_key == "mcp_fortigate_base_url")
                    )
                }
                preserved_vars[f"{key}_local_enabled"] = configured
                preserved_results.append(
                    ApplianceBootstrapResult(
                        key=key,
                        enabled=configured == "true",
                        generated_vars=preserved_vars,
                        secret_content="",
                        access_cidrs=[],
                    )
                )
                print(f"{key}: preserved local setup selection {configured}.")
            elif not inventory_path.exists():
                print(
                    f"{key}: desired by default, but no generated local inventory exists; "
                    "skipping it in non-interactive mode."
                )
        if preserved_results:
            write_text(
                ...
                + "\n"
                + render_appliance_local_vars(preserved_results),
            )
    else:
